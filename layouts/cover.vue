<template>
  <div class="slidev-layout cover text-white" :style="style">
    <div class="my-auto w-full">
      <slot />
      <div
        v-if="coverAuthors.length || coverDate"
        class="absolute bottom-12 flex"
      >
        <p v-if="coverAuthors.length">
          <template v-for="(coverAuthor, idx) in coverAuthors">
            <TextWithOptionalLink
              :link="coverAuthorUrls[idx]"
              :text="coverAuthor"
            />
      ...
      <div
        class="absolute bottom-0 font-extralight mb-1 mr-2 right-0 text-xs"
        v-if="coverBackgroundSource"
      >
        <TextWithOptionalLink
...
const coverAuthorUrls = computed(() =>
  transformIntoArray(coverAuthorUrlTransferred),
);
